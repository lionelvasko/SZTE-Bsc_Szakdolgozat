...
{
    internal class SomfyApiService
    {
        ...
        private static HttpClient CreateHttpClientWithCertificate()
        {
            ...
            {
                Timeout = TimeSpan.FromSeconds(10) // Reduce request timeout if API is slow
            };
        ...
        public List<Entity> GetEntitiesFromJson(string json)
        {
            ...
            if (setupData?.Entities != null)
            {
                ...
                {
                    if (Enum.TryParse<TahomaEntityList>(listEntity.Label, true, out _))
                    {
